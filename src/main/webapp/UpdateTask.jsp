...
<div class="page-wrapper">
  ...
  <form action="UpdateTaskBController" method="post">
  <p><strong>Task ID:</strong> ${join.taskId}${taskId}</p>
  
<input type="hidden" name="taskId" value="${join.taskId}${taskId}">
  
  <strong>Project Title:</strong>
  <select name="projectId" class="form-select mb-3">
  	<c:forEach var="project" items="${projectList}">
      	<option value="${project.projectId}" ${project.title.equals(join.title) || project.projectId == projectId? "selected" : ""}>${project.title}</option>
   	</c:forEach>
 	</select>
 <div class="mb3">
  <strong>Task Detail:</strong> 
  <input type="text" name="taskDetail" class="form-control" value="${join.taskDetail}">
  ...
  <select name="assignedBy" class="form-select mb-3">
          <c:forEach var="user1" items="${userList}">
            <option value="${user1.userId}" ${user1.firstName.concat(user1.lastName).equals(join.assignedByFirstName.concat(join.assignedByLastName)) || user1.userId == assignedBy?"selected":""}>${user1.firstName} ${user1.lastName}</option>
          </c:forEach>
        </select>
  <strong>Assigned To:</strong>
  <select name="assignedTo" class="form-select mb-3">
          <c:forEach var="user1" items="${userList}">
            <option value="${user1.userId}" ${user1.firstName.concat(user1.lastName).equals(join.assignedToFirstName.concat(join.assignedToLastName)) || user1.userId == assignedTo?"selected":""}>${user1.firstName} ${user1.lastName}</option>
          </c:forEach>
        </select>
  <strong>Remarks:</strong> 
  <input type="text" name="remarks" class="form-control mb-3" value="${join.remarks}${remarks}">
  <strong>Status:</strong> 
  
  <select name="status" class="form-select mb-3">
          
     <option value="Initialized" ${join.status.equals("Initialized") || status.equals("Initialized")?"selected":""}>Initialized</option>
     <option value="In Progress" ${join.status.equals("In Progress") || status.equals("In Progress")?"selected":""}>In Progress</option>
     <option value="Pending" ${join.status.equals("Pending") || status.equals("Pending")?"selected":""}>Pending</option>
     <option value="On Hold" ${join.status.equals("On Hold") || status.equals("On Hold")?"selected":""}>On Hold</option>
     <option value="Completed" ${join.status.equals("Completed") || status.equals("Completed")?"selected":""}>Completed</option>
     <option value="Cancelled" ${join.status.equals("Cancelled") || status.equals("Cancelled")?"selected":""}>Cancelled</option>
         
       </select>
  <p><strong>Assigned Date:</strong> ${join.assignedDate}${assignedDate}</p>
  <input type="hidden" name="assignedDate" value="${join.assignedDate}${assignedDate}">
  <p><strong>Completion Date:</strong> ${join.completionDate}${completionDate}</p>
  <input type="hidden" name="completionDate" value="${join.completionDate}${completionDate}">
